...
Raphael.fn.polygon = function (cx, cy, r, n) {
    n = +n < 3 || !n ? 5 : n;
    var points = ["M", cx, cy - r, "L"], polygon, coord = [], coord_x, coord_y, j=0; 
    coord[j++] = {x: cx, y: cy - r};
    for (var i = n-1; i >= 0; i--) {
        coord_x = cx + r * Math.sin(i * Math.PI * 2 / n - Math.PI);
        coord_y = cy + r * Math.cos(i * Math.PI * 2 / n - Math.PI);
        points = points.concat([coord_x, coord_y]);
        coord[j++] = {x: coord_x, y: coord_y};
    }    
    points.push("z");
    polygon = this.path(points);
    polygon.data("coordinates", coord);
    return polygon;
};
